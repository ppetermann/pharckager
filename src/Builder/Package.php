...
use Pharckager\Builder\Bundler\Complete as CompleteBundler;
use Pharckager\Builder\Bundler\Explicit as ExplicitBundler;
use Pharckager\Builder\Package\Autoload;
...
class Package
{
    ...
    public function getBundler(Logger $logger)
    {
        ...
        if ($bundlerName === 'composer') {
            ...
            $logger->log(
                'Invalid bundler "'.$bundlerName.'" specified in package "'.$this->getName(
                ).'", will fall back to "complete" bundler'
            );
