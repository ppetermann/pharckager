...
class Packager
{
    ...
    {
        if ($fn instanceof WriterInterface) {
    ...
    public function assertWritable()
    {
        if (ini_get('phar.readonly') === '1') {
            throw new UnexpectedValueException(
                'Your configuration disabled writing phar files (phar.readonly = On), please update your configuration or run with "php -d phar.readonly=off '.$_SERVER['argv'][0].'"'
            );
        }
    }
    ...
    public function getPharer($path, $version = null)
    {
        ...
        if ($this->isPackageUrl($path)) {
            ...
            $command .= ' create-project '.escapeshellarg($package).' '.escapeshellarg(
                    $path
                ).' --no-dev --no-progress --no-scripts';
            $that = $this;
            $this->displayMeasure(
    ...
    public function exec($cmd, $chdir = null)
    {
        ...
        $process->start();
        $code = $process->wait(
            function ($type, $data) use ($output, &$nl) {
                if ($nl === true) {
                    $data = "\n".$data;
                    $nl = false;
                }
